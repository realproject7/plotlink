...
export function computeWeightedSpend(
  ...
  const eligible = activations
    ...
    .map(a => ({ address: a.address, has_fc_bonus: a.fid !== null ? 1 as const : 0 as const }));
  ...
  for (const p of buyPoints) {
    if (p.action !== "buy" || !eligibleSet.has(p.address)) continue;
  ...
  for (const r of referrals) {
    ...
    if ((buyMap.get(r.referred_address) ?? 0) < config.MIN_REFERRAL_THRESHOLD) continue;
    refCounts.set(r.referrer_address, (refCounts.get(r.referrer_address) ?? 0) + 1);
  }

  const rows: WeightedSpendRow[] = eligibleBuys.map(eb => {
