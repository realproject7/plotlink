...
/**
 * Synchronous Farcaster detection — matches @farcaster/miniapp-sdk's own
 * short-circuit heuristic (ReactNativeWebView for mobile, iframe for web).
 * This is the only viable sync check since hidden() runs at connector-list
 * construction time. The connector itself gracefully fails outside Farcaster.
 */
...
const farcasterWallet = (): Wallet => ({
  ...
  hidden: () => !isLikelyFarcasterContext(),
